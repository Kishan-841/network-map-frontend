...
/**
 * A saved splitter tapped on the draft line (annotate + Pan mode). Bottom
 * anchored, the same idiom as ClosureCard's `edit` mode and TargetCard: what
 * it is, then Edit (the modal) or Remove (the point stays as a plain bend).
 */
export default function SplitterCard({ point, saving, error, onEdit, onRemove, onCancel }) {
  const ref = point.ref ?? {}
  return (
    <div className="absolute bottom-16 left-3 right-3 z-30 mx-auto flex max-w-sm flex-col gap-3 rounded-card border border-line bg-card p-4 shadow-lift sm:bottom-14">
